...
class VinosProvider extends ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    notifyListeners();
        ...
        try {
      final response = await RemoteServices.fetchProductsVinos(
        page: _currentPage,
        limit: _pageSize,
      );
  ...
  Future<void> loadPrevPage() async {
    ...
    List<Products> allProducts = await RemoteServices.fetchProductsVinos(page: 1, limit: 10);
    products = allProducts
        .where((product) =>
            product.category?.name == "Plato fuerte" &&
            product.subcategory?.name == "Vinos")
        .toList();
