...
class PastasProvider extends ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    notifyListeners();
        ...
        try {
      final response = await RemoteServices.fetchProductsPasta(
        page: _currentPage,
        limit: _pageSize,
      );
  ...
  Future<void> loadPrevPage() async {
    ...
    List<Products> allProducts = await RemoteServices.fetchProductsPasta(page: 1, limit: 10);
    products = allProducts
        .where((product) =>
            product.category?.name == "Plato fuerte" &&
            product.subcategory?.name == "Pastas")
        .toList();
