...
class CarneProvider extends ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final response = await RemoteServices.fetchProductsCarne(
        page: _currentPage,
        limit: _pageSize,
      );
  ...
  Future<void> loadPrevPage() async {
    ...
    List<Products> allProducts = await RemoteServices.fetchProductsCarne(page: 1, limit: 10);
    products = allProducts
        .where((product) =>
            product.category?.name == "Plato fuerte" &&
            product.subcategory?.name == "Carnes" &&
            product.status ==true) // condicion
        .toList();
