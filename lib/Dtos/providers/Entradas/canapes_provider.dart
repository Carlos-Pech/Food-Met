...
class CanapesProvider extends ChangeNotifier {
  ...
  Future<void> fetchProducts() async {
    ...
    try {
      final response = await RemoteServices.fetchProductsCanapes(
        page: _currentPage,
        limit: _pageSize,
      );
  ...
  Future<void> loadPrevPage() async {
    ...
    List<Products> allProducts = await RemoteServices.fetchProductsCanapes(page: 1, limit: 10);
    products = allProducts
        .where((product) =>
            product.category?.name == "Entrada" &&
            product.subcategory?.name == "Canapes" &&
